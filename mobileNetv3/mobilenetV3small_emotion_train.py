...
class BatchWiseScaleTransform_train:
    ...
    def __call__(self,batch,gray_flage:bool=False):
        ...
        for img , label in batch:
            ...
            img = letterbox(img, target_size=int(scale+scale/4))
...
class BatchWiseScaleTransform_val:
    ...
    def __call__(self, img, gray_flage:bool=False):
        img = letterbox(img, target_size=96+15)
        self.gray_flage = gray_flage
        if gray_flage:
                img = np.array(img)
                if len(img.shape) == 3:
...
def main():
    ...
    scheduler = ReduceLROnPlateau(optimizer, mode='min', factor=0.8, patience=7)  # 学习率衰减策略
    # 训练循环
