...
class NeuralNet(nn.Module):
    def __init__(self,num_input,num_hidden,num_output):
        ...
        self.bn_conv = nn.BatchNorm2d(out_channels, eps=1e-5, momentum=0.01, affine=True)
        self.pool = nn.MaxPool2d(kernel_size=2, stride=2)
        self.fc1 = nn.Linear(out_channels*14*14,num_hidden)
        self.bn1 = nn.BatchNorm1d(num_hidden, eps=1e-5, momentum=0.1, affine=True)
    ...
    def forward(self,x):
        ...
        x = self.act(x)
        x = self.pool(x)
        x = x.view(-1, self.conv1.out_channels*14*14)
        x = self.fc1(x)
        x = self.bn1(x)
        x = self.act(x)
        x = self.dropout(x)
